        ...
        public StructExpr ParseExp(ref int i)
        {
            ...
            for (i = i; i < toks.Len; i++)
            {
                switch (Get(i).Token)
                {
                    ...
                    case Token.Id:
                        Console.WriteLine("Begun ID:");
                        Log("VarInExpr:" + Get(i).Text, i);
                        Console.WriteLine("ID:" + Get(i).Text);
                        if(Get(i).Text == "loadImg")
                        {
                            int bbb = 4;
                        }
                    ...
                    case Token.Null:
                        ...
                        var null_val = new StructExpr();
                        null_val.Op = OpType.Null;
                        null_val.Type = ExprType.Operator;
                        exp.Expr.Add(null_val);
                        prev_op = false;
